...
async def query(
    ...
    limit: PositiveInt = Query(
        20, alias="limit", description="Quantidade de itens por página"
    ),
    offset: int = Query(0, alias="offset", description="Número de itens para pular"),
) -> Page[list[AtletaOut]]:
    query = select(AtletaModel)
    atletas: list[AtletaOut] =
    (await db_session.execute(query.offset(offset).limit(limit)).scalars().all()
    )

    atleta_list = AtletaOut.model_validate(atleta) for atleta in atletas

    return paginate(atleta_list, limit=limit, offset=offset)
